...
import { ArrowDownTrayIcon, ChartBarIcon, UsersIcon, MagnifyingGlassIcon, PlusIcon, GiftIcon } from '@heroicons/react/24/outline';
import { Fullscreen, FullscreenExit, ContentCopy, CalendarMonth, Cached, Info } from '@mui/icons-material';
  ...
  const getFilteredReferrals = () => {
    ...
    return currentPackReferrals[currentTab].filter(referral => {
      ...
      const searchMatch = searchTerm === '' || 
        ...
        (referral.pack_name && referral.pack_name.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      if (statusFilter === 'active') {
        ...
        statusMatch = referral.pack_status === 'expired' || 
                     (referral.expiry_date && normalizeDate(referral.expiry_date) < new Date());
      }
      ...
      if (startDate && endDate) {
        ...
        const dateField = dateFilter.type === 'purchase' ? referral.purchase_date : referral.expiry_date;
    ...
    <Container sx={{ py: 4 }}>
      ...
      {userPacks.length === 0 ? (
        ...
        >
          ...
          <Typography variant="body1" sx={{ maxWidth: '500px', mb: 2, color: 'text.secondary' }}>
            Ajoutez des packs pour permettre aux utilisateurs de s'inscrire et de parrainer d'autres membres.
          </Typography>
          <Button
            variant="contained"
                    ...
                    <TextField
                      ...
                      sx={{
                        ...
                        '& .MuiOutlinedInput-root': {
                          ...
                          bgcolor: isDarkMode ? '#1a2433' : 'rgba(255, 255, 255, 0.9)',
                          '&:hover': {
                            bgcolor: isDarkMode ? '#1a2433' : 'rgba(255, 255, 255, 1)'
                          },
                  ...
                  >
                    <Box sx={{ 
                      ...
                      <Box>
                        ...
                        <Typography variant="h5" sx={{ fontWeight: 600, color: isDarkMode ? 'primary.light' : 'primary.main' }}>
                    ...
                    {viewMode === 'table' && (
                      <div className="relative" ref={exportMenuRef} style={{ marginLeft: 'auto' }}>
                        <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                          <Button
                            variant="outlined"
                            onClick={() => setShowExportMenu(!showExportMenu)}
                            startIcon={<ArrowDownTrayIcon className="h-5 w-5" />}
                            endIcon={showExportMenu ? <ChevronUpIcon className="h-4 w-4" /> : <ChevronDownIcon className="h-4 w-4" />}
                            sx={{
