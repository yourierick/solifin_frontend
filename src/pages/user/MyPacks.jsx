...
const MyPacks = () => {
  ...
  // Filtrer les filleuls en fonction de la recherche
  const filteredReferrals = React.useMemo(() => {
    if (!currentPackReferrals) return [];
    
    // S'assurer que currentPackReferrals[currentTab] existe, sinon retourner un tableau vide
    const currentGeneration = currentPackReferrals[currentTab] || [];

    const referrals = currentGeneration.map((referral) => ({
      ...referral,
      id: referral.id || `${currentTab}-${referral.user_id}`,
      totalCommission: parseFloat(referral.total_commission || 0).toFixed(2)
    }));
    ...
    return referrals.filter(referral => 
      ...
      referral.pack_code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      referral.totalCommission.toString().includes(searchTerm) ||
      new Date(referral.created_at).toLocaleDateString().includes(searchTerm)
    );
  }, [currentPackReferrals, currentTab, searchTerm]);
  ...
  const transformDataToTree = (referrals) => {
    ...
    if (referrals[0]) {
      ...
      // Deuxième génération
      if (referrals[1]) {
        rootNode.children.forEach(gen1Node => {
          const gen2Refs = referrals[1].filter(ref => 
            ref.sponsor_id === gen1Node.attributes.userId
          );
          gen1Node.children = gen2Refs.map(ref => ({
            name: ref.name,
            attributes: {
              commission: `${parseFloat(ref.total_commission || 0).toFixed(2)}$`,
              status: ref.pack_status,
          ...
          if (referrals[2]) {
            gen1Node.children.forEach(gen2Node => {
              const gen3Refs = referrals[2].filter(ref =>
                ref.sponsor_id === gen2Node.attributes.userId
              );
              gen2Node.children = gen3Refs.map(ref => ({
                name: ref.name,
                attributes: {
                  commission: `${parseFloat(ref.total_commission || 0).toFixed(2)}$`,
                  status: ref.pack_status,
              ...
              if (referrals[3]) {
                gen2Node.children.forEach(gen3Node => {
                  const gen4Refs = referrals[3].filter(ref =>
                    ref.sponsor_id === gen3Node.attributes.userId
                  );
                  gen3Node.children = gen4Refs.map(ref => ({
                    name: ref.name,
                    attributes: {
                      commission: `${parseFloat(ref.total_commission || 0).toFixed(2)}$`,
                      status: ref.pack_status,
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      >
        <DialogTitle sx={{ 
          ...
          <Typography variant="h6">Arbre des filleuls</Typography>
