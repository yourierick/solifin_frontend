...
export default function RenewPackForm({ open, onClose, pack, onRenew }) {
  ...
  useEffect(() => {
    if (pack) {
      ...
      setTotalAmount(getPackPrice() * step);
    }
  }, [pack]);
  ...
  const validateForm = () => {
    ...
    if (paymentMethod === PAYMENT_TYPES.WALLET) {
      ...
      isValid =
        isValid && walletBalance >= totalAmount + (transactionFees || 0);
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <Typography variant="h5" component="h2" className="font-bold" id="renew-pack-dialog-title">
